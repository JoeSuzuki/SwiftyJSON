...
internal func exerciseOne() {
    ...
    let tittle = userData["results"][0]["name"]["tittle"].stringValue
    let emailAddress = userData["results"][0]["email"].stringValue
    let cellPhoneNumber = userData["results"][0]["cell"].stringValue
    print("\(firstName) \(lastName) lives at \(streetName) in \(city), \(state), \(postCode). If you want to contact \(tittle).  \(lastName), you can email \(emailAddress) or call at \(cellPhoneNumber). ")
